...
#### SAR ML
munnell_SAR_ML<-lagsarlm(fm, data=munnell_cross, listw=usalw, type = "lag")
...
S_pcap <- iIrW %*% (COEF2*diag(nrow(munnell_cross)))
S_pc <- iIrW %*% (COEF3*diag(nrow(munnell_cross)))
...
for ( i in 1:nrow(samplesA1)) {# loop through simulated parameters values.
  ...
  simu_effectA1_pcap <- iIrW %*% ((samplesA1[i,4]*diag(nrow(munnell_cross)))) 
  ...
  simu_effectA1_pc <- iIrW %*% ((samplesA1[i,5]*diag(nrow(munnell_cross)))) 
