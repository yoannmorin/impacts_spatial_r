...
I_sar_p <- impacts(sar_p, listw=usalw, time=17, zstats=TRUE, R=1000)
print(I_sar_p )
summary(I_sar_p , zstats=TRUE)
...
S_pcap <- iIrW %*% (COEF2*diag(nrow(Produc)))
S_pc <- iIrW %*% (COEF3*diag(nrow(Produc)))
...
for ( i in 1:nrow(samplesA1)) {# loop through simulated parameters values.
  ...
  simu_effectA1_pcap <- iIrW %*% ((samplesA1[i,2]*diag(nrow(Produc)))) 
  ...
  simu_effectA1_pc <- iIrW %*% ((samplesA1[i,3]*diag(nrow(Produc)))) 
