...
##########################################################################
######                                                               #####
######     Impact calculation in spatial econometrics models in R    #####
######                                                               #####
##########################################################################
##########################################################################
...
#### SDM ML
munnell_SAR_ML<-lagsarlm(fm, data=munnell_cross, listw=usalw, type = "mixed")
...
I_munnell_SAR_ML<-impacts(munnell_SAR_ML, listw=usalw, zstats=TRUE, R=10000)
print(I_munnell_SAR_ML)
summary(I_munnell_SAR_ML, zstats=TRUE)
...
COEF8 <- munnell_SAR_ML$coefficients[8] ### spatial lag log(emp)
COEF9 <- munnell_SAR_ML$coefficients[9] ### spatial lag unemp
...
S_emp <- iIrW %*% ((COEF4*diag(nrow(munnell_cross)))+ (COEF8*listw2mat(usalw)))
S_unemp <- iIrW %*% ((COEF5*diag(nrow(munnell_cross)))+ (COEF9*listw2mat(usalw)))
...
for ( i in 1:nrow(samplesA1)) { # loop through simulated parameters values.
  ...
  simu_effectA1_emp <- iIrW %*% ((samplesA1[i,6]*diag(nrow(munnell_cross)))+ (samplesA1[i,10]*listw2mat(usalw))) 
  ...
  simu_effectA1_unemp <- iIrW %*% ((samplesA1[i,7]*diag(nrow(munnell_cross)))+ (samplesA1[i,11]*listw2mat(usalw))) 
