...
###############################################################
######                                                    #####
######    Impact calculation in Panel SAR models in R     #####
######                                                    #####
###############################################################
###############################################################
...
# Compute spatial lags of DURBIN variable

usalw_p <- as.matrix(bdiag(replicate(17, listw2mat(usalw), simplify = FALSE)))
usap_lw <- mat2listw(usalw_p)

Produc$log_pcap <- log(Produc$pcap)
Produc$log_pc <- log(Produc$pc)

Produc$slag_log_pcap <- lag(usap_lw, Produc$log_pcap)
Produc$slag_log_pc <- lag(usap_lw, Produc$log_pc)
...
fm <- log(gsp) ~ log(pcap) + log(pc) + log(emp) + unemp + slag_log_pcap + slag_log_pc

# We estimate a SAR within model with twoways effects

sar_p <- spml(formula = fm, data = Produc, lag=TRUE, spatial.error="none", listw = usalw,  model = "within", effect="twoways")
summary(sar_p)
...
COEF1 <- sar_p$coefficients[1] ### rho
COEF2 <- sar_p$coefficients[2] ### log(pcap)
COEF3 <- sar_p$coefficients[3] ### log(pc)
COEF4 <- sar_p$coefficients[4] ### log(emp)
COEF5 <- sar_p$coefficients[5] ### unemp
COEF6 <- sar_p$coefficients[6] ### spatial lag of log(pcap)
COEF7 <- sar_p$coefficients[7] ### spatial lag of log(pc)


# Inverse (I - rho W)^-1
usalw_p <- as.matrix(bdiag(replicate(17, listw2mat(usalw), simplify = FALSE)))

iIrW <- invIrW(usalw_p, COEF1)
...
S_pcap <- iIrW %*% ((COEF2*diag(nrow(Produc)))+ (COEF6*usalw_p))
S_pc <- iIrW %*% ((COEF3*diag(nrow(Produc)))+ (COEF7*usalw_p))
S_emp <- iIrW %*% (COEF4*diag(nrow(Produc)))
S_unemp <- iIrW %*% (COEF5*diag(nrow(Produc)))


### Direct impacts ###
dir_pcap <- sum(diag(S_pcap))/nrow(Produc)
dir_pc <- sum(diag(S_pc))/nrow(Produc)
dir_emp  <- sum(diag(S_emp))/nrow(Produc)
dir_unemp <- sum(diag(S_unemp))/nrow(Produc)


### Total impacts ####

tot_pcap <- sum(c(S_pcap))/nrow(Produc)
tot_pc <- sum(c(S_pc))/nrow(Produc)
tot_emp <- sum(c(S_emp))/nrow(Produc)
tot_unemp <- sum(c(S_unemp))/nrow(Produc)
...
select_rho_beta <- sar_p$coefficients   # select rho and coefficients
mu <- c(select_rho_beta)


Sigma <- sar_p$vcov # select variance-covariance matrix
...
check <- seq(1,nrow(samplesA1),10) # sequence to print progress in the loop

usalw_p <- as.matrix(bdiag(replicate(17, listw2mat(usalw), simplify = FALSE))) # Spatial interraction matrix for panel case
...
for ( i in 1:nrow(samplesA1)) {# loop through simulated parameters values.
  ...
  iIrW <- invIrW(usalw_p, samplesA1[i,1]) # Inverse (I - rho W)^-1
  
  
  # Each bloc compute for one of the variable : the (dense) S(W)_r matrices, direct impact, total impact and indirect impact.
  
  simu_effectA1_pcap <- iIrW %*% ((samplesA1[i,2]*diag(nrow(Produc)))+ (samplesA1[i,6]*usalw_p))
  simuA1_pcap[i,1] <- sum(diag(simu_effectA1_pcap))/nrow(Produc) 
  simuA1_pcap[i,2] <- sum(c(simu_effectA1_pcap))/nrow(Produc)
  simuA1_pcap[i,3] <- simuA1_pcap[i,2] - simuA1_pcap[i,1] 
  
  simu_effectA1_pc <- iIrW %*% ((samplesA1[i,3]*diag(nrow(Produc)))+ (samplesA1[i,7]*usalw_p))
  simuA1_pc[i,1] <- sum(diag(simu_effectA1_pc))/nrow(Produc) 
  simuA1_pc[i,2] <- sum(c(simu_effectA1_pc))/nrow(Produc)
  simuA1_pc[i,3] <- simuA1_pc[i,2] - simuA1_pc[i,1] 
  
  simu_effectA1_emp <- iIrW %*% ((samplesA1[i,4]*diag(nrow(Produc)))) 
  simuA1_emp[i,1] <- sum(diag(simu_effectA1_emp))/nrow(Produc) 
  simuA1_emp[i,2] <- sum(c(simu_effectA1_emp))/nrow(Produc)
  simuA1_emp[i,3] <- simuA1_emp[i,2] - simuA1_emp[i,1] 
  
  simu_effectA1_unemp <- iIrW %*% ((samplesA1[i,5]*diag(nrow(Produc)))) 
  simuA1_unemp[i,1] <- sum(diag(simu_effectA1_unemp))/nrow(Produc) 
  simuA1_unemp[i,2] <- sum(c(simu_effectA1_unemp))/nrow(Produc)
